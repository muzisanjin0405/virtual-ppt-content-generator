...
$updaterRoot = Split-Path -Parent $PSScriptRoot
$sourceConfigPath = Join-Path $updaterRoot 'references\update-source.json'
if (-not (Test-Path -LiteralPath $sourceConfigPath -PathType Leaf)) {
    throw 'Missing references/update-source.json.'
}

$sourceConfig = Get-Content -Raw -LiteralPath $sourceConfigPath | ConvertFrom-Json
$Repository = [string]$sourceConfig.repository
$skillName = [string]$sourceConfig.skill_name
$releaseApi = [string]$sourceConfig.release_api
$assetPrefix = [string]$sourceConfig.asset_prefix

if ($Repository -ne 'muzisanjin0405/virtual-ppt-content-generator') {
    throw "Unexpected update repository: $Repository"
}
if ($skillName -ne 'virtual-ppt-content-generator') {
    throw "Unexpected target Skill: $skillName"
}
if ($releaseApi -ne 'https://api.github.com/repos/muzisanjin0405/virtual-ppt-content-generator/releases/latest') {
    throw "Unexpected release API: $releaseApi"
}
if ($assetPrefix -ne 'virtual-ppt-content-generator-v') {
...
try {
    ...
    if ($statusCode -eq 404) {
        throw 'The official repository has no published stable Release yet.'
    }
    throw
}
...
$archiveName = "$assetPrefix$releaseVersionText.zip"
...
if ($null -ne $currentVersion -and $currentVersion -ge $releaseVersion) {
    Write-Host "Installed version: v$currentVersion"
    Write-Host "Latest release:   v$releaseVersion"
...
if ($null -ne $currentVersion) {
    ...
    Write-Host 'Installed version: not detected'
}
Write-Host "Latest release:   v$releaseVersion"

if ($CheckOnly) {
    Write-Host "Update available: v$releaseVersion"
    return
}
